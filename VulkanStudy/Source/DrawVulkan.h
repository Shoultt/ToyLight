...
void updateMVP()
{
	...
	glm::mat4 model = glm::translate(glm::rotate(glm::scale(glm::mat4(1.0f), glm::vec3(3.0f)), glm::radians(9.0f), glm::vec3(0.0f, 1.0f, 0.0f)), glm::vec3(-2.0f, -5.0, -6.0f));
	...
	.view = view,
	.proj = projection,              
	.lightPos = glm::rotate(glm::mat4(1.0f), glm::radians(50.0f) * (float)glfwGetTime(), glm::vec3(0.0f, 1.0f, 0.0f)) * glm::vec4(40.0f, 4.0f, 55.0f, 0.0f) };
...
void presentFrame()
{
	...
	if (timeFrames - 1 == secondaryTime)
	{
		windowTitle = std::to_string(framesCount);
		framesCount = 0;
		secondaryTime = timeFrames;
	}
}
