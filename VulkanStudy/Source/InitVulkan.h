...
private:
	...
	void graphPipilines_create()
	{
		...
		const VkVertexInputAttributeDescription vertexAttributeDsns[] = {positionAttributeDsn, normalAttributeDsn};
		...
		graphPipeline_create(vsModule, fsModule, pipeCI, graphPipeline0, false);
		...
		.format = VK_FORMAT_R32G32B32_SFLOAT,
		.offset = sizeof(float) * 3 };
		const VkVertexInputAttributeDescription vertexAttributeDsns1[] = { vertexAttributeDsn1, normalAttributeDsn1 };
		...
		graphPipeline_create(polyVSmodule, polyFSmodule, pipeCI1, graphPipeline1, true);
	}
